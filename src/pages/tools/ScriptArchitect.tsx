...
export default function ScriptArchitect() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authorizedFetch('/api/tools/script-architect', {
        method: 'POST',
        body: JSON.stringify({ topic, formula, platform }),
          ...
          <div className="rounded-xl bg-white dark:bg-gray-800 p-6 shadow-sm ring-1 ring-gray-100 dark:ring-gray-700">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                {loading ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Sedang Membuat Naskah...
                  </>
                ) : (
                  'Buat Naskah'
                )}
